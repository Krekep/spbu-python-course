...
def test_num_threads():
    ...
    assert len(pool.threads) == num_threads, f"Expected {num_threads} threads, found {len(pool.threads)}"
    pool.dispose()
...
def test_enqueue_tasks():
    ...
    assert total_time < 0.5, f"Tasks should have been executed concurrently, but took {total_time:.2f}s"
    ...
    assert len(completed_tasks) == n, f"Expected {n} tasks completed, got {len(completed_tasks)}"
