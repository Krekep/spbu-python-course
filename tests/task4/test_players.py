...
class TestHumanPlayer:
    ...
    @patch('builtins.input', side_effect=['x', 'h'])  # Сначала неверный ввод, потом верный
    ...
    @patch('builtins.input', side_effect=['-50', '100'])  # Сначала отрицательная, потом верная
    ...
    @patch('builtins.input', side_effect=['not_a_number', '150'])  # Сначала не число, потом верное
...
class TestDealer:
    ...
    def test_dealer_reveal_hand(self, sample_dealer):
        ...
        expected_text = f"Карты дилера: {sample_dealer.hand} (Сумма: {sample_dealer.hand.value})"
