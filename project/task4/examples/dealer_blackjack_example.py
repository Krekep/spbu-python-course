...
class GuaranteedBlackjackDeck(Deck):
    ...
    def deal_card(self):
        ...
        if self.current_round == 2 and self.card_count <= len(self.blackjack_round_cards):
            card = self.blackjack_round_cards[self.card_count - 1]
            print(f"🎯 Контролируемая выдача: {card} (карта #{self.card_count} в раунде {self.current_round})")
            return card
        
        # Для всех остальных случаев используем обычную логику
        card = super().deal_card()
        print(f"🎲 Случайная карта: {card} (карта #{self.card_count} в раунде {self.current_round})")
        return card
...
def run_dealer_blackjack_example():
    ...
    for player in final_state['players']:
        ...
        if len(game.round_history) >= 2:
            ...
            player_after_1 = next((p for p in after_round1['players'] if p['name'] == player['name']), None)
            
            if player_after_1:
                change_round2 = player['chips'] - player_after_1['chips']
                trend = "📈" if change_round2 > 0 else "📉" if change_round2 < 0 else "➡️"
                print(f"   {trend} {player['name']}: {player['chips']} фишек (изменение во 2 раунде: {change_round2:+})")
