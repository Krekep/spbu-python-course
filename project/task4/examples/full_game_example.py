...
def run_full_game_example():
    ...
    for round_num in range(game.max_rounds):
        ...
        if round_num > 0:
            ...
            for current_player in current_state['players']:
                previous_player = next((p for p in previous_state['players'] if p['name'] == current_player['name']), None)
                if previous_player:
    ...
    for i, player in enumerate(sorted_players, 1):
        ...
        performance = "Отлично" if profit_percent > 20 else "Хорошо" if profit_percent > 0 else "Плохо"
        print(f"   {medal} {player['name']}: {player['chips']} фишек ({profit:+}, {profit_percent:+.1f}%) - {performance}")
    ...
    for i in range(1, total_rounds + 1):
        ...
        # Статистика раунда
        active_players = len([p for p in round_info['final_state']['players'] if p['chips'] > 0])
